...
// // ── Main export ───────────────────────────────────────────────────────────────
// export default function Discovery({ data }: { data: IndividualProduct["thirdSection"] }) {
...
function TabPill({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`px-20 sm:px-[21px] py-[7px] sm:py-[10px] max-[440px]:h-[34px] 3xl:h-[48px] rounded-[50px] border text-description tracking-[-0.03em] font-semibold transition-all duration-300 cursor-pointer min-w-[65px] md:min-w-[112px] ${
        active ? "bg-secondary text-white border-secondary" : "bg-transparent text-secondary border-secondary"
      }`}
    >
      {label}
    </button>
      ...
      <div className="container">
        <div className="flex flex-col 2xl:flex-row 2xl:items-center gap-20 md:gap-40 2xl:gap-70 3xl:gap-[113px] border-b border-[#D7D7D7] pb-20 sm:pb-40 2xl:pb-120 3xl:pb-150">
          ...
          <div className="w-full flex flex-col">
            ...
            <div className="flex flex-wrap gap-x-[10px] gap-y-[15px] sm:gap-20 mb-[30px] md:mb-40 3xl:mb-50">
              {data.items.map((t, i) => (
                <Reveal key={t._id} variants={moveUpV2} delayRange={i * 0.11}>
                  <TabPill label={t.category} active={i === activeTab} onClick={() => setActiveTab(i)} />
                </Reveal>
              ))}
            </div>
                ...
                {tab.items.length > 0 && (
                  <>
                    ...
                    <div className="hidden 2xl:flex flex-col">
                      ...
                      <div className="grid grid-cols-3 gap-x-50 3xl:gap-x-80 gap-y-20">
                        {tab.items.map((h, i) => (
                          <Reveal key={h._id} variants={moveUpV2} delayRange={i * 0.11}>
                            <HighlightItem icon={h.icon} title={h.title} />
                          </Reveal>
                        ))}
                      </div>
          ...
          {tab.items.length > 0 && (
            <div className="2xl:hidden pt-[10px]">
              <h3 className="text-subHeading tracking-[-0.03em] mb-20">Highlights</h3>
              <div className="flex flex-wrap sm:grid sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-x-40 gap-y-20">
                {tab.items.map((h, i) => (
                  <Reveal key={h._id} variants={moveUpV2} delayRange={i * 0.11}>
                    <HighlightItem icon={h.icon} title={h.title} />
