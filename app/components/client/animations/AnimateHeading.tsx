...
export function AnimatedHeading({
  ...
  const ref = useRef<HTMLHeadingElement>(null);
  const isSingleWordBlade =
    mode === "blade" && title.trim().split(/\s+/).length === 1;
  ...
  // ── Tokenise: split on newlines first, then spaces ──────────────────────
  type Token = { type: "word"; value: string } | { type: "br" };
const tokens: Token[] = title.split(/(\n)/).flatMap<Token>((chunk) =>
    chunk === "\n"
      ? [{ type: "br" as const }]
      : chunk
          .split(" ")
          .filter(Boolean)
          .map((w) => ({ type: "word" as const, value: w })),
  );
  ...
  return (
    <h1 ref={ref} className={`text-heading ${className}`}>
      ...
      {wholeElModes.has(mode)
        ...
        : mode === "split"
          ? tokens.map((token, wi) => {
              if (token.type === "br") return <br key={`br-${wi}`} />;
              const word = token.value;
              const isLast = wi === tokens.length - 1;
              return (
                <span key={wi} style={{ display: "inline" }}>
                  <span
                    className="ah-word-wrap"
          ...
          : tokens.map((token, wi) => {
              ...
              const word = token.value;
              const isLast = wi === tokens.length - 1;
              const nextIsBr = tokens[wi + 1]?.type === "br";
              const spacer =
                !isLast && !nextIsBr ? (
                  <span key={`sp-${wi}`} style={{ display: "inline" }}>
                    {" "}
